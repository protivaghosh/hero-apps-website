...
import { addToApp, getStoreApp } from '../../Utility/AddtoApp';
import {
...
const AppDetails = () => {
  ...
  if (!app) {
    return (
      <div className="flex flex-col justify-center items-center h-screen bg-gray-50 text-gray-700">
        <img src={errorImg} alt="App Not Found" className="w-48 h-48 mb-6 opacity-80" />
  ...
  const [installed, setInstalled] = useState(() => getStoreApp().includes(id));

  
  useEffect(() => {
    const handleStorageChange = () => {
      setInstalled(getStoreApp().includes(id));
    };
      ...
      <div className="max-w-4xl mx-auto bg-white p-6 rounded-lg shadow-xl">
        ...
        <div className="flex items-start mb-6">
          <div className="w-20 h-20 bg-gray-200 rounded-xl flex items-center justify-center mr-4 flex-shrink-0 overflow-hidden">
            <img src={image} alt={`${title} icon`} className="object-cover w-full h-full" />
          </div>
          <div className="flex flex-col">
            ...
            <p className="text-xs text-gray-400 mt-1">Size: {size}MB</p>
          </div>
        </div>
